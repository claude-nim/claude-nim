...
export async function* streamChatCompletion(
  ...
  // Fixed-size ring buffer for SSE lines — avoids string concatenation per chunk.
  // Only the last incomplete line is retained between chunks.
  let incompleteLine = "";
  ...
  function extractLines(text: string): string[] {
    ...
    for (let i = 0; i < text.length; i++) {
      if (text[i] === "\n") {
        lines.push(incompleteLine + text.slice(start, i));
        incompleteLine = "";
        start = i + 1;
      }
    }
    if (start < text.length) {
      incompleteLine = text.slice(start);
    }
    return lines;
  }
  ...
  const processLine = (line: string): OcGoStreamResponse | null => {
    const trimmed = line.trim();
    if (!trimmed.startsWith("data: ")) return null;
    const data = trimmed.slice(6);
    if (data === "[DONE]") return null;
    try {
      return JSON.parse(data) as OcGoStreamResponse;
    } catch {
      return null;
    }
  };
  ...
  try {
    while (true) {
      ...
      const lines = extractLines(decoder.decode(value, { stream: true }));
      for (let i = 0; i < lines.length; i++) {
        const parsed = processLine(lines[i]);
        if (parsed) yield parsed;
      }
    }

    // Flush decoder internal state and process remaining incomplete line
    const remaining = decoder.decode();
    if (remaining) {
      incompleteLine += remaining;
    }
    if (incompleteLine) {
      const parsed = processLine(incompleteLine);
      if (parsed) yield parsed;
      incompleteLine = "";
    }
  } catch (error) {
    ...
    try {
      ...
      // ignore — reader may already be cancelled/released from timeout
    }
  }
}
